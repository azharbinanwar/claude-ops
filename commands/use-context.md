---
description: Work from what's already in this conversation — don't re-read
argument-hint: [task]
disable-model-invocation: true
---
Do this using the code and facts already present in this conversation — files read earlier, results already shown, decisions already made. Do not re-read, re-search, or re-verify what is already here.

Open a file only if something you genuinely need was never shown, or you're about to edit a region you haven't seen — then read just that region. If you believe context is stale (file changed since it was read), say which file in one line instead of silently re-reading everything.

Task: $ARGUMENTS
