...
4. Present two real selectable options using the option-picker tool, not plain-text yes/no:
   - **Commit** — stages exactly the files listed above (never `git add -A` or `git add .`), then runs `git commit` with the exact message shown, via heredoc. Report the resulting commit hash. Does not push. Nothing else.
